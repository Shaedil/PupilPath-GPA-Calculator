...
function action() {
    ...
    for (var termElement of document.getElementsByClassName("transcript")) {
        ...
        for (var courseElement of termElement.rows) {
            //grab the details
            var DBN = courseElement.cells[1].innerHTML;
            if (DBN == "13K430") {
    ...
    var gpa = Math.round(((gradeAccumulator / creditAccumulator) + Number.EPSILON) * 1000) / 1000;
    var injectedHTML = '<div><span><mark><b>';
    injectedHTML += 'GPA: ';
    injectedHTML += gpa;
    injectedHTML += '</b></mark></span></div>';
    //document.getElementById('transcript_container').insertAdjacentHTML('afterend', injectedHTML);
    document.getElementById('TranBy').insertAdjacentHTML('afterend', injectedHTML);
